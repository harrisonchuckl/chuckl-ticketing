...
const DOC_TYPES = new Set<Prisma.PromoterDocumentType>([
  "PRS_CERTIFICATE",
  "PPL_MUSIC_LICENSING",
...
function normaliseDocumentType(value: unknown): Prisma.PromoterDocumentType {
  const candidate = String(value || "OTHER").toUpperCase();
  if (DOC_TYPES.has(candidate as Prisma.PromoterDocumentType)) {
    return candidate as Prisma.PromoterDocumentType;
  }
  return "OTHER";
}
...
router.post("/promoters/:promoterId/documents", requireAdminOrOrganiser, async (req, res) => {
  try {
    ...
    const type = normaliseDocumentType(req.body?.type);
    if (!title) {
...
router.post("/promoters/documents/upload", requireAdminOrOrganiser, async (req, res) => {
  try {
    ...
    const put = await uploadToR2(key, fileBuffer, {
