...
const CheckoutReq = z.object({
  showId: z.string(),
  items: z.array(
    z.object({ ticketTypeId: z.string(), quantity: z.number().int().min(1).max(10) })
  ),
  email: z.string().email()
});

router.post('/create', async (req: Request, res: Response) => {
  const body = CheckoutReq.safeParse(req.body);
  ...
  for (const it of items) {
    const tt = show.ticketTypes.find(t => t.id === it.ticketTypeId);
    ...
    data: {
      ...
      items: {
        create: items.map(i => ({
          ...
          quantity: i.quantity,
          unitPence: show.ticketTypes.find(t => t.id === i.ticketTypeId)!.pricePence
        }))
      }
    }
  });
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: items.map(i => {
      const tt = show.ticketTypes.find(t => t.id === i.ticketTypeId)!;
      return {
        price_data: {
          currency: process.env.STRIPE_CURRENCY || 'gbp',
          product_data: { name: `${show.title} — ${tt.name}` },
    ...
    success_url: `${process.env.PUBLIC_BASE_URL}/success?orderId=${order.id}`,
    cancel_url: `${process.env.PUBLIC_BASE_URL}/events/${show.id}`
  });
