    ...
    try {
        ...
        function processNode(node, parentGroup) {
            ...
            if (nodeType === 'Circle' && node.getAttr('isSeat')) {
                ...
                if (isUnavailable) {
                    ...
                    seat.shadowColor('#00000033'); seat.shadowBlur(6); seat.shadowOpacity(0.1);
        ...
        function showSeatTooltip(id) {
            ...
            tooltip.innerHTML = `
              ...
              <span class="tt-meta">${meta.ticketName} • £${(meta.price/100).toFixed(2)}</span>
              ${meta.info ? `<div class="tt-info">${meta.info}</div>` : ''}
              ${meta.viewImg ? `<img src="${meta.viewImg}" alt="Seat view" />` : ''}
            `;
            tooltip.style.display = 'block';
        }

        // --- 3. ADD UI ICONS ---
        const showViews = false; // default hidden until toggle
        const uiIcons = [];
        function updateIcons() {
            ...
            seatMeta.forEach((meta, seatId) => {
                ...
                const groupScale = parentGroup ? parentGroup.getAbsoluteScale().x : 1;
