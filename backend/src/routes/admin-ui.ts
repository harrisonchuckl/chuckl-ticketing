      ...
      +         '</div>'
      +       '</div>'
      +       '<div id="upsells-page" style="display: none;">'
      +         '<div style="text-align: center; max-width: 600px; margin: 80px auto; background: white; padding: 80px 40px; border-radius: 24px; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.1);">'
      +           '<div style="font-size: 64px; margin-bottom: 24px;">💰</div>'
    ...
    function setActiveTab(target, page){
      document.querySelectorAll('.nav-tab').forEach(function(tab){
        tab.style.background = 'white';
        tab.style.color = '#4a5568';
        tab.style.border = '2px solid #e2e8f0';
      });
      if (target){
      ...
      var pages = ['overview', 'create', 'store', 'orders', 'settings', 'upsells'];
      pages.forEach(function(key){
        var el = document.getElementById(key + '-page');
    ...
    document.querySelectorAll('.nav-tab').forEach(function(tab){
      tab.addEventListener('click', function(e){
        ...
        subtitle.textContent = 'Navigating to ' + e.target.textContent + '...';
