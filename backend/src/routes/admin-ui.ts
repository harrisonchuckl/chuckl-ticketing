...
export default router; but the admin-ui seemed to cut of early before you produced all the code: // backend/src/routes/admin-ui.ts
import { Router } from "express";
...
(function(){
  ...
  function mountVenuePicker(input){
    ...
    function render(list,q){
      ...
      if(list.length){
        list.forEach(v=>{
          ...
          el.addEventListener('click',()=>{ input.value=v.name; input.dataset.venueId=v.id; close(); });
          pop.appendChild(el);
        });
      }
    ...
    input.addEventListener('focus', async ()=>{ const q=input.value.trim(); if(!q) return; render(await searchVenues(q), q); });
    ...
    async function load(){
      ...
      try{
        ...
        tb.innerHTML = items.map(s=>{
          ...
          const total = (s._alloc?.total ?? 0), sold=(s._alloc?.sold ?? 0), hold=(s._alloc?.hold ?? 0), avail=Math.max(total-sold-hold,0);
  ...
  async function ticketsPage(id){
    ...
    // Seat maps UI (summary on tickets page)
  ...
  async function seatingPage(showId){
        ...
        +'<div class="grid grid-2">'
          +'<div class="grid" style="align-content:start;gap:12px">'
            ...
            +'<div class="card" style="margin:0">'
              ...
              +'<div class="muted" style="font-size:12px;margin-bottom:6px">Generate a simple rectangular layout. You can tweak individual seats afterwards.</div>'
              ...
              +'</div>'
              +'<div class="grid grid-2" style="margin-bottom:6px">'
                +'<div class="grid"><label>Row labels</label>'
                  +'<select id="q_rowMode">'
                    +'<option value="letters">A, B, C…</option>'
                    +'<option value="numbers">1, 2, 3…</option>'
                  +'</select>'
                +'</div>'
                +'<div class="grid"><label>First row label</label><input id="q_rowStart" value="A"/></div>'
              +'</div>'
              +'<button class="btn p" id="q_generate" style="margin-top:6px;width:100%">Generate seats into selected map</button>'
              +'<div class="tip" style="margin-top:4px;font-size:12px">This will create seats only where they don\'t already exist (based on row/seat number).</div>'
              +'<div class="error" id="q_err" style="margin-top:4px"></div>'
            +'</div>'
          ...
          +'<div class="grid" style="align-content:start;gap:12px">'
            ...
            +'<div class="seat-layout-wrap">'
              ...
              +'</div>'
              +'<div id="seatGridContainer" style="margin-top:8px"></div>'
              ...
              +'<div class="row-between" style="margin-top:4px">'
                +'<div class="muted" id="seatSummary" style="font-size:12px">No seats loaded.</div>'
                +'<label style="font-size:12px;display:flex;align-items:center;gap:4px"><input type="checkbox" id="toggleSeatNumbers"/> Hide numbers</label>'
              +'</div>'
            +'</div>'

            +'<div class="card" style="margin:0">'
              +'<div class="title" style="margin-bottom:4px">Seat inspector</div>'
              +'<div class="muted" id="seatInspectorHint" style="font-size:12px;margin-bottom:6px">Click a seat in the map to edit its details.</div>'
              +'<div id="seatMeta" style="display:none">'
                +'<div class="grid grid-2" style="margin-bottom:6px">'
                  +'<div class="grid"><label>Row label</label><input id="meta_rowLabel"/></div>'
                  +'<div class="grid"><label>Seat number</label><input id="meta_seatNumber" type="number" min="0"/></div>'
                +'</div>'
                +'<div class="grid grid-2" style="margin-bottom:6px">'
                  +'<div class="grid"><label>Level / area</label><input id="meta_level"/></div>'
                  +'<div class="grid"><label>Zone</label><input id="meta_zoneId"/></div>'
                +'</div>'
                +'<div class="grid" style="margin-bottom:6px">'
                  +'<label>Label (full)</label><input id="meta_label" placeholder="e.g. Stalls A12"/>'
                +'</div>'
                +'<div class="row" style="margin-bottom:6px">'
                  +'<button class="btn p" id="metaSave">Save details</button>'
                  +'<button class="btn" id="metaToggleBlocked">Toggle blocked</button>'
                  +'<button class="btn" id="metaToggleHeld">Toggle held</button>'
                +'</div>'
                +'<div class="muted" id="metaStatus" style="font-size:12px"></div>'
              +'</div>'
            +'</div>'
    ...
    async function loadSeatMaps(){
      ...
      smSelect.innerHTML = '';
      activeSeatMapId = null;
      seatsCache = [];
      seatGridContainer.innerHTML = '';
      seatSummary.textContent = 'No seats loaded.';
    ...
    smSelect.addEventListener('change', function(){
      var v = smSelect.value || '';
      activeSeatMapId = v || null;
      if(activeSeatMapId){
        loadSeats();
      }else{
        seatGridContainer.innerHTML = '';
    ...
    $('#sm_create').addEventListener('click', async function(){
      ...
      try{
        ...
        if(created && created.ok && created.id){
          smName.value = '';
          await loadSeatMaps();
          activeSeatMapId = created.id;
          smSelect.value = created.id;
          loadSeats();
        }else{
          smErr.textContent = (created && created.error) || 'Failed to create seat map';
        }
      }catch(e){
    ...
    function computeRowLabel(index, mode, start){
      if(mode === 'numbers'){
        var base = parseInt(start, 10);
        if(isNaN(base)) base = 1;
        return String(base + index);
      }
      // letters
      var s = (start || 'A').toUpperCase();
      var baseCode = s.charCodeAt(0);
      if(isNaN(baseCode)) baseCode = 65;
      return String.fromCharCode(baseCode + index);
    }

    $('#q_generate').addEventListener('click', async function(){
      qErr.textContent = '';
      if(!activeSeatMapId){
      ...
      if(!Number.isFinite(rows) || rows <= 0 || !Number.isFinite(cols) || cols <= 0){
        qErr.textContent = 'Rows and seats per row must be positive numbers.';
        return;
      }
      ...
      for(var r=0;r<rows;r++){
        ...
        for(var c=1;c<=cols;c++){
          seats.push({
            ...
            rowLabel: rowLabel,
            seatNumber: c,
            label: rowLabel + String(c)
          });
        }
      }

      try{
        await j('/seatmaps/'+encodeURIComponent(activeSeatMapId)+'/seats/bulk', {
          method:'POST',
          headers:{'Content-Type':'application/json'},
    ...
    function renderSeats(){
      ...
      if(!seatsCache.length){
        seatSummary.textContent = 'No seats in this map yet.';
        return;
      }
      var byRow = {};
      seatsCache.forEach(function(s){
        var r = s.row || s.rowLabel || '';
        if(!byRow[r]) byRow[r] = [];
        byRow[r].push(s);
      });
      var rows = Object.keys(byRow).sort(function(a,b){
        if(a < b) return -1;
        if(a > b) return 1;
        return 0;
      });
      ...
      rows.forEach(function(rowKey){
        var rowSeats = byRow[rowKey].slice().sort(function(a,b){
          var na = a.number || a.seatNumber || 0;
          var nb = b.number || b.seatNumber || 0;
          return na-nb;
        });
        ...
        labelSpan.textContent = rowSeats[0].rowLabel || rowKey || '';
        rowDiv.appendChild(labelSpan);

        var grid = document.createElement('div');
        grid.className = 'seat-grid';
        grid.style.gridTemplateColumns = 'repeat('+rowSeats.length+', 1fr)';

        rowSeats.forEach(function(s){
          var btn = document.createElement('div');
          var cls = 'seat';
          if(s.status === 'BLOCKED') cls += ' seat-blocked';
          else if(s.status === 'HELD') cls += ' seat-held';
          else if(s.status === 'SOLD') cls += ' seat-sold';
          btn.className = cls;
          if(hideNumbers) btn.classList.add('seat-number-hidden');
          btn.dataset.seatId = s.id;
          var span = document.createElement('span');
          span.textContent = String(s.seatNumber || s.number || '');
    ...
    function selectSeat(seatId){
      ...
      $$('.seat', seatGridContainer).forEach(function(el){
        el.classList.toggle('seat-selected', el.dataset.seatId === seatId);
      });
    ...
    async function loadSeats(){
      ...
      selectedSeatId = null;
      setInspectorVisible(false);
      seatGridContainer.innerHTML = '<div class="muted" style="font-size:12px">Loading seats…</div>';
      if(!activeSeatMapId){
        seatGridContainer.innerHTML = '<div class="muted" style="font-size:12px">Select a seat map to view seats.</div>';
        return;
      }
      try{
        var data = await j('/seatmaps/'+encodeURIComponent(activeSeatMapId)+'/seats');
        seatsCache = Array.isArray(data) ? data : [];
        renderSeats();
      }catch(e){
        seatGridContainer.innerHTML = '<div class="error" style="font-size:12px">Failed to load seats: '+(e.message||e)+'</div>';
      }
    }

    toggleSeatNumbers.addEventListener('change', function(){
      $$('.seat', seatGridContainer).forEach(function(el){
        el.classList.toggle('seat-number-hidden', toggleSeatNumbers.checked);
      });
    });
    ...
    metaSave.addEventListener('click', async function(){
      metaStatus.textContent = '';
      if(!selectedSeatId){
        metaStatus.textContent = 'No seat selected.';
        return;
      }
      var body = {
      ...
      try{
        await j('/seatmaps/seat/'+encodeURIComponent(selectedSeatId)+'/meta', {
          method:'PATCH',
          headers:{'Content-Type':'application/json'},
        ...
        metaStatus.textContent = 'Failed to save: '+(e.message||e);
      }
    });
    ...
    async function updateSeatStatus(targetStatus){
      ...
      try{
        await j('/seatmaps/seat/'+encodeURIComponent(selectedSeatId)+'/status', {
          method:'PATCH',
          headers:{'Content-Type':'application/json'},
        ...
        metaStatus.textContent = 'Failed to update status: '+(e.message||e);
      }
    }

    metaToggleBlocked.addEventListener('click', function(){
      var seat = seatsCache.find(function(s){ return s.id === selectedSeatId; });
      if(!seat) return;
      var next = seat.status === 'BLOCKED' ? 'AVAILABLE' : 'BLOCKED';
      updateSeatStatus(next);
    });

    metaToggleHeld.addEventListener('click', function(){
      var seat = seatsCache.find(function(s){ return s.id === selectedSeatId; });
      if(!seat) return;
      var next = seat.status === 'HELD' ? 'AVAILABLE' : 'HELD';
      updateSeatStatus(next);
    });
  ...
  function orders(){
    main.innerHTML =
      '<div class="card">'
        ...
        +'<div class="muted">Orders view coming soon.</div>'
      +'</div>';
  }
  ...
  function venues(){
    main.innerHTML =
      '<div class="card">'
        ...
        +'<div class="muted">Venues management coming soon.</div>'
      +'</div>';
  }
