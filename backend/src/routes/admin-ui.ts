...
router.get(['/ui', '/ui/', '/ui/home', '/ui/*'], requireAdminOrOrganiser, (_req, res) => {
  // Avoid stale cached JS while iterating
...
(function(){
  ...
  function route(){
    // IMPORTANT: single slashes in regex; trim one optional trailing slash
    ...
    // Robust (no over-escaping) matchers for show subpages:
  ...
  // -------- Create / Edit show (shell only; existing endpoints power these) --------
