  ...
  function venues(){
    ...
    function renderVenues(items){
      ...
      (items || []).forEach(function(v){
        ...
        var spaces = Array.isArray(v.spaces) ? v.spaces.slice() : [];
        var maps = Array.isArray(v.seatingMaps) ? v.seatingMaps.slice() : [];
        var contraPct = v.ticketContraBps != null ? v.ticketContraBps / 100 : null;
        var feePct = v.bookingFeeBps != null ? v.bookingFeeBps / 100 : 10;
        ...
        card.innerHTML = ''
          ...
          +     '<input type="url" placeholder="https://image-url" data-field="image" value="' + escapeHtml(v.imageUrl || '') + '" />'
          ...
          +       (v.imageUrl ? '<img src="' + escapeHtml(v.imageUrl) + '" alt="Preview" style="width:100%;height:100%;object-fit:cover;" />' : '<div class="muted">No image yet</div>')
          +     '</div>'
          +   '</div>'

          +   '<div class="grid" style="gap:6px;">'
          +     '<label style="margin:0;font-weight:600;font-size:13px;">Contact name<input data-field="contactName" value="' + escapeHtml(v.contactName || '') + '" placeholder="Venue manager" /></label>'
          +     '<label style="margin:0;font-weight:600;font-size:13px;">Contact email<input data-field="contactEmail" type="email" value="' + escapeHtml(v.contactEmail || '') + '" placeholder="manager@example.com" /></label>'
          +     '<label style="margin:0;font-weight:600;font-size:13px;">Contact phone<input data-field="contactPhone" value="' + escapeHtml(v.contactPhone || '') + '" placeholder="+44 20 1234 5678" /></label>'
          +   '</div>'

          +   '<div class="grid" style="gap:6px;">'
          +     '<label style="margin:0;font-weight:600;font-size:13px;">Capacity<input type="number" min="1" data-field="capacity" value="' + escapeHtml(String(v.capacity || '')) + '" /></label>'
          +     '<label style="margin:0;font-weight:600;font-size:13px;">Ticket contra (%)<input type="number" min="0" max="100" step="0.5" data-field="contra" value="' + escapeHtml(contraPct != null ? String(contraPct) : '') + '" placeholder="e.g. 20" /></label>'
          +     '<label style="margin:0;font-weight:600;font-size:13px;">Booking fee (%)<input type="number" min="10" step="0.5" data-field="fee" value="' + escapeHtml(feePct != null ? String(feePct) : '10') + '" /></label>'
          ...
          +     '<div class="error" data-err="' + escapeHtml(v.id) + '" style="font-size:12px;"></div>'
        ...
        if (saveBtn){
          saveBtn.addEventListener('click', async function(){
            ...
            var errBox = card.querySelector('[data-err="' + v.id + '"]');
            ...
            var data = {
              ...
              ticketContraPercent: numberOf('contra'),
              bookingFeePercent: Math.max(10, numberOf('fee') || 10),
              spaces: spaces.slice(),
              seatingMaps: maps.slice(),
            };
            try{
              await j('/admin/venues/' + v.id, {
                method:'PUT',
                headers:{'Content-Type':'application/json'},
              ...
              if (errBox){ errBox.textContent = (e && e.message) || 'Failed to save'; }
            }finally{
