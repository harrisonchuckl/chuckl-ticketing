...
/**
 * IMPORTANT:
 * This router is mounted at /admin/ui in server.ts.
 * Therefore the paths here are relative to /admin/ui.
 * We serve the same SPA shell for ANY subpath under /admin/ui.
 */
router.get(
  [
    "/",              // /admin/ui
    ...
    "/audiences",
    "/email",
    "/shows",         // /admin/ui/shows (list)
    "/shows/*",       // /admin/ui/shows/...
    "/*"              // catch-all: future pages
  ],
...
(function(){
  try {
    ...
    function route(){
      ...
      const path = pathname.startsWith(BASE) ? pathname : BASE;

      setActive(path);

      if (path === BASE || path === BASE+'/home' || path === BASE+'/index.html') return home();
      if (path === BASE+'/shows/create') return createShow();
      if (path === BASE+'/shows/current') return listShows();
      if (path === BASE+'/orders') return orders();
      if (path === BASE+'/venues') return venues();

      // specific show pages
      if (path.startsWith(BASE+'/shows/') && path.endsWith('/seating')) {
        const id = path.split('/')[4];
        return seatingPage(id);
      }
      if (path.startsWith(BASE+'/shows/') && path.endsWith('/edit')) {
        const id = path.split('/')[4];
        return editShow(id);
      }
    ...
    async function listShows(){
      ...
      async function load(){
        ...
        $$('[data-edit]').forEach(a=>a.addEventListener('click',(e)=>{ e.preventDefault(); go(BASE+'/shows/'+a.getAttribute('data-edit')+'/edit'); }));
        $$('[data-seating]').forEach(a=>a.addEventListener('click',(e)=>{ e.preventDefault(); go(BASE+'/shows/'+a.getAttribute('data-seating')+'/seating'); }));
        $$('[data-dup]').forEach(a=>a.addEventListener('click', async (e)=>{
          ...
          if(r.ok && r.newId){ go(BASE+'/shows/'+r.newId+'/edit'); }
        }));
      }
    ...
    async function editShow(id){
      ...
      $('#goSeating').addEventListener('click',(e)=>{e.preventDefault(); go(BASE+'/shows/'+id+'/seating');});
      $('#save').addEventListener('click', async ()=>{
        const payload = {
          title: $('#sh_title').value.trim(),
          date: $('#sh_dt').value ? new Date($('#sh_dt').value).toISOString() : null,
    ...
    if (main) {
      main.innerHTML = '<div class="card"><div class="title">Error</div><pre class="error">'+String(err.stack||err)+'</pre></div>';
    }
    console.error(err);
  }
})();
