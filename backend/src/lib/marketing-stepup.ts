...
const STEPUP_COOKIE = "marketing_stepup";
const STEPUP_TTL_MS = Number(process.env.MARKETING_STEPUP_TTL_MS || 10 * 60 * 1000);
...
function signStepUp(userId: string) {
  ...
  return jwt.sign({ sub: userId, type: "marketing_stepup" }, JWT_SECRET, { expiresIn: seconds });
}

function verifyStepUp(token: string, userId: string) {
  const payload = jwt.verify(token, JWT_SECRET) as any;
  return payload?.type === "marketing_stepup" && String(payload.sub || "") === userId;
}

export function setMarketingStepUpCookie(req: Request, res: Response, userId: string) {
...
export function requireMarketingStepUp(req: Request, res: Response, next: NextFunction) {
  if (!req.user) {
